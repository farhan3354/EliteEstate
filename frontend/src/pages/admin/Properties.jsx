...
//         fetchProperties(); // Refresh list
//         Swal.fire({
//           title: !isFeatured ? "Featured!" : "Unfeatured!",
//           text: `Property ${!isFeatured ? "featured" : "unfeatured"} successfully`,
//           icon: "success",
//           timer: 2000,
//           showConfirmButton: false,
//         });
...
//           text: `Property ${!isVerified ? "verified" : "unverified"} successfully`,
//           icon: "success",
//           timer: 2000,
//           showConfirmButton: false,
//         });
...
//       Swal.fire("Error", "Failed to update verified status", "error");
//     }
//   };
...
const AdminProperties = () => {
  ...
  const fetchProperties = async () => {
    try {
      ...
      const response = await api.get("/properties/admin", { params });
      ...
      Swal.fire({
        title: "Error!",
        text: error.response?.data?.message || "Failed to load properties. Please try again.",
        icon: "error",
  ...
  }, [pagination.page, filterStatus, filterType, filterCategory]);
  ...
  const updatePropertyStatus = async (propertyId, status) => {
    try {
      ...
      console.error("Error updating property status:", error);
      Swal.fire("Error", error.response?.data?.message || "Failed to update property status", "error");
    }
  };
  ...
  const toggleFeatured = async (propertyId, isFeatured) => {
    try {
      ...
      console.error("Error toggling featured:", error);
      Swal.fire("Error", error.response?.data?.message || "Failed to update featured status", "error");
    }
  };
  ...
  const toggleVerified = async (propertyId, isVerified) => {
    try {
      ...
      console.error("Error toggling verified:", error);
      Swal.fire("Error", error.response?.data?.message || "Failed to update verified status", "error");
    }
  };
  ...
  const deleteProperty = async (propertyId) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        console.error("Error deleting property:", error);
        Swal.fire("Error", error.response?.data?.message || "Failed to delete property", "error");
      }
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600 mt-2">Manage all property listings in the system</p>
        </div>
        <button
          onClick={fetchProperties}
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        ...
        <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200">
          <div className="text-center">
            <p className="text-2xl font-bold text-yellow-600">{stats.rentedProperties}</p>
            <p className="text-gray-600">Rented</p>
          </div>
        </div>
        <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200">
          <div className="text-center">
            <p className="text-2xl font-bold text-gray-600">{stats.inactiveProperties}</p>
            <p className="text-gray-600">Inactive</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
              ...
              {properties.length === 0 ? (
                ...
                properties.map((property) => (
                  <tr key={property._id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <div className="text-sm text-gray-900">
                        <div className="capitalize">{property.category || 'N/A'}</div>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex flex-col space-y-2">
                        ...
                        <div className="flex space-x-2">
                          <button
                            onClick={() => toggleFeatured(property._id, property.isFeatured)}
                          ...
                          <button
                            onClick={() => toggleVerified(property._id, property.isVerified)}
                            className={`px-3 py-1 text-xs rounded transition-colors ${
                              property.isVerified
                                ? 'bg-blue-200 text-blue-800 hover:bg-blue-300'
                                : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                            }`}
                            title={property.isVerified ? "Unverify property" : "Verify property"}
                          >
        ...
        {pagination.pages > 1 && (
          <div className="flex items-center justify-between px-6 py-4 border-t border-gray-200">
            <div className="text-sm text-gray-700">
              Showing page {pagination.page} of {pagination.pages} • Total {pagination.total} properties
            </div>
            <div className="flex space-x-2">
              <button
